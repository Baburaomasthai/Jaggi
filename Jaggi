...
def get_user_settings(user_id):
    ...
    if result:
        return {
            'user_id': result[0],
            'custom_caption': result[1],
            'protect_content': result[2],
            'no_forward': result[3],
            'auto_delete': result[4],
            'auto_delete_time': result[5]
        }
    return None
...
async def handle_files(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if 'creating_folder' in context.user_data and context.user_data['creating_folder']:
        ...
        if settings and settings['custom_caption']:
            ...
            caption = caption.replace('{file_name}', item["file_name"])
            caption = caption.replace('{file_size}', str(item["file_size"]))
            caption = caption.replace('{caption}', item["content"])
...
async def done_folder(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if 'creating_folder' in context.user_data and context.user_data['creating_folder']:
        ...
        no_forward = settings['no_forward'] if settings else 0
        ...
        # Create folder entry
        cursor.execute(
            'INSERT INTO folders (user_id, folder_code, protect_content, no_forward) VALUES (?, ?, ?, ?)',
            (user_id, folder_code, protect_content, no_forward)
        )
...
async def start_with_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if len(context.args) > 0:
        ...
        cursor = conn.cursor()
        cursor.execute(
            'SELECT folder_id, protect_content, no_forward FROM folders WHERE folder_code = ?',
            (link_code,)
        )
        folder_result = cursor.fetchone()
        
        if folder_result:
            folder_id, protect_content, no_forward = folder_result
            ...
            cursor.execute(
                'SELECT message_type, content, file_id, protect_content, no_forward FROM messages WHERE link_code = ?',
                (link_code,)
            )
            ...
            if message_result:
                message_type, content, file_id, protect_content, no_forward = message_result
...
async def shortener(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Simple URL shortening (in a real implementation, use a URL shortening service API)
    short_url = f"https://short.url/{hash(url) % 1000000}"
...
async def settings(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("No Forward", callback_data="setting_no_forward")],
    ...
    status_text = (
        ...
        f"• No Forward: {'Enabled' if settings and settings['no_forward'] else 'Disabled'}\n\n"
...
async def settings_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "setting_caption":
        await query.edit_message_text(
            "Send me your custom caption template. You can use these placeholders:\n\n"
            "• {file_name} - File name\n"
            "• {file_size} - File size\n"
            "• {caption} - Original caption\n\n"
            "Example: `{file_name} | Size: {file_size} bytes`\n\n"
        ...
        await query.edit_message_text(
            ...
            "To change the auto delete time, send me the number of seconds (e.g., 30).\n"
            "Send /cancel to keep the current time."
        )
        context.user_data['awaiting_auto_delete_time'] = True
        
    elif query.data == "setting_no_forward":
        current = settings['no_forward'] if settings else 0
        new_value = 0 if current else 1
        
        update_user_settings(user_id, 'no_forward', new_value)
        status = "enabled" if new_value else "disabled"
        
        await query.edit_message_text(f"No Forward has been {status}.")
    ...
    if 'awaiting_caption' in context.user_data:
        ...
        if text == '/cancel':
            ...
            try:
                ...
                await update.message.reply_text(f"Auto delete time has been set to {time} seconds.")
            except ValueError:
...
def main():
    ...
    # Add handlers
    application.add_handler(CommandHandler("start", start_with_link, filters=filters.Regex(r'^/start(\s|$)')))
    ...
    application.add_handler(CallbackQueryHandler(settings_callback, pattern="^setting_|^remove_channel_|^add_channel|^settings_back$"))
