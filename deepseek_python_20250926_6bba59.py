...
class AdvancedAutoForwardBot:
    ...
    async def start_message_listener(self, user_id: int):
        """Enhanced message listener using USER CLIENT for source channel"""
        try:
            if user_id not in self.user_clients or user_id not in self.source_channels:
                logger.warning(f"❌ Cannot start listener for user {user_id}: client or source channel missing")
                return
            ...
            user_client = self.user_clients[user_id]  # Use USER client, not bot client
            
            logger.info(f"🔰 Starting message listener for user {user_id} on channel {source_channel_id}")
            
            @user_client.on(events.NewMessage(chats=source_channel_id))
            async def message_handler(event):
                try:
                    # Check if auto forwarding is active
                    ...
                    # Process and forward the message using BOT client for targets
    ...
    async def forward_message_to_target(self, user_id: int, message, target: Dict, processed_text: str, settings: Dict) -> bool:
        """Enhanced message forwarding with media support - using BOT CLIENT for targets"""
        try:
            # Use BOT client to send to target channels (bot needs admin in targets)
            ...
            if (settings.get('forward_media', True) and 
                ...
                # Download media from user client and forward using bot client
                user_client = self.user_clients[user_id]
                
                # Download media file
                media_path = await user_client.download_media(message.media, file="downloads/")
                
                if media_path:
                    # Forward media with caption using BOT client
                    ...
                    # Fallback to text only if media download fails
...
This account has 2FA enabled. Please temporarily disable 2FA to use this bot.
            """)
...
**Note:** The bot will read messages using YOUR account access.
                """
...
**Important:** Make sure the bot has admin rights in this channel!
                """
            ...
            buttons = [
                [Button.inline("📊 Dashboard", b"show_dashboard")],
                [Button.inline("➕ Add More" if selection_type == 'target' else "🎯 Add Targets", 
                             b"manage_targets" if selection_type == 'source' else "set_source")]
            ]
...
• Media Forwarding: {'✅' if settings.get('forward_media', True) else '❌'}
• URL Previews: {'✅' if settings.get('url_previews', True) else '❌'}
• Caption Forwarding: {'✅' if settings.get('caption_forward', True) else '❌'}
• Remove Usernames: {'✅' if settings.get('remove_usernames', False) else '❌'}
• Remove Links: {'✅' if settings.get('remove_links', False) else '❌'}
• Hide Header: {'✅' if settings.get('hide_header', False) else '❌'}

**Advanced:**
• Delay: {settings.get('delay_seconds', 1)}s
• Max Message Length: {settings.get('max_message_length', 4000)} chars
        """
...
This bot helps you automatically forward messages from one channel to multiple other channels.

**Key Features:**
• Source channel access via YOUR account (no bot admin needed)
• Target channels managed by bot (bot needs admin)
...
Click **Login / Start** to begin!
    ...
    async def toggle_auto_forwarding(self, event):
        ...
        if user.id not in self.source_channels:
            await event.answer("❌ Please set a source channel first", alert=True)
            return
        
        if user.id not in self.target_channels or not self.target_channels[user.id]:
            await event.answer("❌ Please add target channels first", alert=True)
            return
...
You can login again anytime using `/login`
            ...
            await event.edit(logout_text, buttons=[[Button.inline("🔐 Login Again", b"start_login")]])
...
**🔧 Key Features:**
• **Source Channels**: Accessed via YOUR account (no bot admin needed)
• **Target Channels**: Bot needs admin rights to send messages
• **Media Support**: Forward photos, videos, documents
• **Text Processing**: Remove usernames/links, word replacement

**⚙️ Setup Process:**
1. Login with your account
2. Set source channel (bot reads via your account)
3. Add target channels (bot needs admin rights)
4. Configure settings
5. Start auto-forwarding

**Important Notes:**
• Source channel: User account reads messages
• Target channels: Bot sends messages (needs admin)
• Media files are downloaded and re-uploaded for forwarding
        """
...
• **Important**: You must have access to read this channel

**Step 3: Add Target Channels**  
• Click "Manage Targets" in dashboard
• Select channels you want to forward TO
• **Important**: The bot needs admin rights in these channels
...
**Architecture:**
• 📡 Source → Your Account (reads messages)
• 🤖 Bot → Target Channels (sends messages)
        """
