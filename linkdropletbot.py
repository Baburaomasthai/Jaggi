...
async def handle_files(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if 'creating_folder' in context.user_data and context.user_data['creating_folder']:
        ...
        # Apply custom caption if set (ONLY add custom text, don't replace file info)
        if settings and settings['custom_caption']:
            # Just use the custom caption as-is without any replacements
        ...
        if folder_result:
            ...
            if message_result:
                ...
                if settings and settings['auto_delete']:
                    ...
                    try:
                        await msg.delete()
                        await update.message.reply_text(f"Message deleted after {settings['auto_delete_time']} minutes.")
                    except Exception as e:
                        logger.error(f"Error deleting message: {e}")
            else:
