...
Send now or type 'cancel' to abort.
    ...
    async def process_message_content_with_entities(self, message, user_id: int, settings: Dict):
        """Process message content while preserving entities"""
        try:
    ...
    async def forward_single_message_with_entities(self, user_client, message, target_id: int, processed_data: Dict, settings: Dict):
        """Forward a single message to target with entities preservation"""
        try:
...
Choose an option below:
    ...
    async def handle_callback(self, event):
        ...
        try:
            ...
            if data == "start_login":
                # Fix for login button - use original handle_login but with event
                await self.handle_login_from_button(event)
