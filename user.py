...
Send now or type 'cancel' to abort.
    ...
    async def process_and_forward_message(self, user_id: int, message):
        """Process and forward message to target channels - FIXED VERSION"""
        try:
            ...
            processed_text = await self.process_message_content(message, user_id, settings)
            
            # Forward message based on type
            for target in self.target_channels[user_id]:
                try:
                    await self.forward_single_message(user_client, message, target['id'], processed_text, settings)
    ...
    async def process_message_content(self, message, user_id: int, settings: Dict) -> str:
        """Process message content with replacements and settings - FIXED"""
        try:
    ...
    async def forward_single_message(self, user_client, message, target_id: int, processed_text: str, settings: Dict):
        """Forward a single message to target - FIXED VERSION"""
        try:
            # Handle different message types
            if message.media:
                # Media message (photo, video, document, etc.)
                if processed_text:  # Has caption/text
