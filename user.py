...
You can send it now or use: `/login +919876543210`
    ...
    async def handle_login_button(self, event):
        ...
        if not await self.check_force_subscribe(user.id):
            await event.edit("❌ Please join our channel first to use this bot.")
            return
        
        if user.id in self.user_sessions:
            await event.edit("✅ You are already logged in!")
...
**Total target channels:** {len(self.target_channels[user_id])}
    ...
    async def process_message_text(self, user_id: int, text: str) -> str:
        """Process message text with word and link replacements"""
        if not text:
            return ""
        ...
        if user.id in self.user_sessions:
            buttons.extend([
                ...
                [Button.inline("🆘 Help", b"show_help"),
                 Button.inline("🗑️ Logout", b"logout_confirm")]
            ])
        else:
...
Toggle the settings below:
        ...
        buttons = [
            [Button.inline(f"📝 Hide Header: {'✅ ON' if settings['hide_header'] else '❌ OFF'}", 
                          b"toggle_hide_header")],
            [Button.inline(f"🖼️ Forward Media: {'✅ ON' if settings['forward_media'] else '❌ OFF'}", 
                          b"toggle_forward_media")],
            [Button.inline(f"🔗 URL Previews: {'✅ ON' if settings['url_previews'] else '❌ OFF'}", 
                          b"toggle_url_previews")],
            [Button.inline(f"👤 Remove Usernames: {'✅ ON' if settings['remove_usernames'] else '❌ OFF'}", 
                          b"toggle_remove_usernames")],
            [Button.inline(f"🌐 Remove Links: {'✅ ON' if settings['remove_links'] else '❌ OFF'}", 
    ...
    async def word_replace_menu(self, event):
        ...
        if replacements:
            ...
            if len(replacements) > 5:
                menu_text += f"• ... and {len(replacements) - 5} more\n"
        else:
    ...
    async def check_user_logged_in(self, user_id: int, event) -> bool:
        ...
        if user_id not in self.user_sessions:
            if hasattr(event, 'edit'):
                ...
                await event.reply("❌ Please login first using /login command")
            return False
        return True
...
• Enable "Post Messages" permission
    ...
    async def logout_confirm(self, event):
        """Confirm logout"""
        confirm_text = """
🚪 **Logout Confirmation**
...
You can login again anytime.
    ...
    async def handle_logout(self, event):
        ...
        try:
            ...
            await event.edit("✅ Logout successful! All your data has been cleared.\n\nSend /start to use the bot again.")
            
        except Exception as e:
            logger.error(f"Error during logout: {e}")
            await event.edit("❌ Error during logout. Please try again.")
...
Click the button below to begin!
            ...
            if self.is_admin(user.id):
                buttons.append([Button.inline("📊 Admin Stats", b"show_stats")])
        ...
        async def logout_handler(event):
            await self.logout_confirm(event)
        ...
        @self.client.on(events.NewMessage(pattern=r'^AUTOX\d+$'))
        async def code_handler(event):
            await self.handle_code_verification(event)

        # Word replacement input handler
        @self.client.on(events.NewMessage(func=lambda e: e.is_private and e.sender_id in self.awaiting_word_replacement))
        async def word_replace_handler(event):
            await self.handle_word_replacement(event)

        # Link replacement input handler
        @self.client.on(events.NewMessage(func=lambda e: e.is_private and e.sender_id in self.awaiting_link_replacement))
        ...
        async def button_handler(event):
            ...
            try:
                ...
                elif data == "view_all_words":
                    replacements = self.word_replacements.get(user.id, {})
                    if replacements:
                        text = "**Your Word Replacements:**\n\n"
                        for orig, rep in replacements.items():
                            text += f"• `{orig}` → `{rep}`\n"
                        await event.edit(text)
                    else:
                        await event.edit("❌ No word replacements set.")
                ...
                elif data == "view_all_links":
                    replacements = self.link_replacements.get(user.id, {})
                    if replacements:
                        text = "**Your Link Replacements:**\n\n"
                        for orig, rep in replacements.items():
                            text += f"• `{orig}` → `{rep}`\n"
                        await event.edit(text)
                    else:
                        await event.edit("❌ No link replacements set.")
